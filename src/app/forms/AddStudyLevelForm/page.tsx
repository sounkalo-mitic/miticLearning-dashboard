...
import { createStudyLevel } from "@/services/studyLevelService";
import React, { useState } from "react";

const AddStudyLevelForm: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            // Envoi de la requête pour créer un niveau d'étude
            await createStudyLevel({ name, description });
            setName(""); // Réinitialiser le champ "nom"
            setDescription(""); // Réinitialiser le champ "description"
            alert("Niveau d&apos;étude ajouté avec succès !"); // Confirmation à l'utilisateur
        } catch (err: any) {
    ...
    return (
        <DefaultLayout>
            ...
            >
                {/* Titre du formulaire */}
                <h2 className="text-xl font-bold">Ajouter un level d&apos;étude</h2>
...
export default AddStudyLevelForm;
